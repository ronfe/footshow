...
newProbTable <- function(homeAttack, homeDefence, awayAttack, awayDefence, homeFactor){
  lambdaA <- exp(homeAttack - awayDefence + homeFactor)
  lambdaB <- exp(awayAttack - homeDefence)
  A <- as.numeric()
  B <- as.numeric()
  ...
  return(unlist(round(C, 8), use.names=F))
  #colnames(C) <- name
...
ProbTable <- function(parameters,hometeam,awayteam) {
  ...
  lambdaa <- exp(P[a,]$Attack - P[b,]$Defence)
...
ResultProbs <- function(probs) {
  ...
  return(list(ArsenalWin=R[1], Draw=R[2], ChelseaWin=R[3]))
}
